...
applicationMasterCtrl.applicationMasterActiveDeactive = (req, res) => {
    ...
    ApplicationMasterModel.updateOne(query, { $set: { status: req.body.status } }, function (err, applicationMaster) {
        if (err) {
...
applicationMasterCtrl.applicationMasterDetailsById = (req, res) => {
    ...
    try {
        ...
        ApplicationMasterModel.advancedAggregate(query, {}, (err, applicationMaster) => {
            if (err) {
                throw err;
                    ...
                    if (applicationMaster === null) {
                        ...
                        ApplicationMasterModel.update(query, bodydata, function (err, applicationMaster) {
                            if (err) {
                                ...
                                response.setError(AppCode.Fail);
                                response.send(res);
                            } else if (applicationMaster == undefined || (applicationMaster.matchedCount === 0 && applicationMaster.modifiedCount === 0)) {
                                response.setError(AppCode.NotFound);
                            } else {
            ...
            if (applicationMaster == null) {
                ...
                ApplicationMasterModel.remove(query, function (err, applicationMaster) {
                    if (err) {
                        ...
                        response.setError(AppCode.Fail);
                        response.send(res);
                    } else if (applicationMaster == undefined || applicationMaster.deletedCount === 0) {
                        AppCode.Fail.error = "No record found";
                        response.setError(AppCode.Fail);
                        response.send(res);
                    } else {
